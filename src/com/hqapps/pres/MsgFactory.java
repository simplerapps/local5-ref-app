...
public abstract class MsgFactory 
{
	...
	public ReqMsg getRequestMsg ( Method method, HttpServletRequest req )
	{
		...
		try {
			...
			Gson gson = new Gson();
			// NEW: general way to get resource msg
			Class cls = MsgProcessor.getResourceClass(resource);
			if ( cls != null ) {
				ReqMsg msg = (ReqMsg)gson.fromJson(jsonMsg, cls);
				msg.setResource(resource, cls);
				return msg;
			}
			else if ( resource.equals( RS_USER ) ) {
			    User msg = gson.fromJson(jsonMsg, User.class);
			    msg.setResource (resource, null);
			    return msg;
			}
		...
		unkownMsg.setResource(resource, null);
		return unkownMsg;
	} 
