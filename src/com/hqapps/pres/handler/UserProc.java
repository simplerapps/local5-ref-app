...
public class UserProc extends MsgProcessor 
{
			...
			throws Exception
	{
		...
		// Save user object
			...
			if ( em.load(user.getEmail(), User.KIND) != null ) {
				...
				// store the user info
				em.storeNew(user.getEmail(), User.KIND, values);
				
				User authUser = new User();
				authUser.fromDataMap(values);
				authUser.setLoginToken(
						PassReset.createSecurityToken(user.getEmail(), passwordEnc));
	 ...
	 * @param user
	 */
	private void checkSvcAdmin ( User user ) throws Exception
	{
		String adminSvcs = "";
		
		// if superuser allow admin on all services
		if ( AdminUtils.isSuperAdmin( user.getEmail()) ) {
			adminSvcs = "SUPER-USER";
		}
		else {
			adminSvcs = UserCache.getInst().getAdminServices(user.getEmail());
		}
		// return response
		user.setAdminSvcs(adminSvcs);
	}
	...
	private RespMsg resetPassword ( EntityManager em, User user )
	{
		try {
			...
			String decResetTok = PassReset.decodeToken ( resetTok );
			String email = PassReset.getEmailFromTok ( decResetTok );
			 ...
			 // store the user info
			 em.storeNew(email, User.KIND, entity.getProperties());
